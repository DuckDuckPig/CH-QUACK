...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 14 11:22:36 2024
...
# ACWE Folders
saveFolderScaled  = '/mnt/coronal_holes/Paper 2/Code 02 Observations/'
saveFolderScaled  = saveFolderScaled + 'HMI_Experiments/TestEvolutionMethods/Scaled/'   # Update to reflect Dataset Location
saveFolderDefault = '/mnt/coronal_holes/Paper 2/Code 02 Observations/'
saveFolderDefault = saveFolderDefault + 'HMI_Experiments/TestEvolutionMethods/Standard/' # Update to reflect Dataset Location

# ACWE Compare Prefix
coreACWEprefix = 'ACWEresize_param1.' # Prefix for ACWE which we
                                      # will compare all others to
otherPrefix = ['ACWE.'] # ['ACWEresize_param2.','ACWEresize_param4.','ACWE.']
...
crSaveFolder1 = saveFolderScaled  + magMethod + '/' + CR + '/'
crSaveFolder2 = saveFolderDefault + magMethod + '/' + CR + '/'
...
for i in range(len(data[keys[acweChoice]])):
    ...
    if coreACWE != None:
        ...
        # Check for Errors
        alphas[i,0,0] = np.max(AHC['INIT_ALPHA'])
        alphas[i,0,1] = np.max(AHC['ALPHA'])
        ...
        for ACWEfile in ACWEfiles:
            ...
            for j in range(len(otherPrefix)):
                ...
                if otherPrefix[j] in ACWEfile:
                    ...
                    # Check for Errors
                    alphas[i,j+1,0] = np.max(AH['INIT_ALPHA'])
                    alphas[i,j+1,1] = np.max(AH['ALPHA'])
...
resultsFolder = 'HMI_Experiments/TestEvolutionMethods/Scaled/Analysis_HMI/Results/'
resultsFolder = os.path.join(ROOT_DIR,resultsFolder)
if not os.path.exists(resultsFolder):
    os.makedirs(resultsFolder)
filename = os.path.basename(CarringtonFile) + '.' + magMethod + '.ScaleStats.npz'
