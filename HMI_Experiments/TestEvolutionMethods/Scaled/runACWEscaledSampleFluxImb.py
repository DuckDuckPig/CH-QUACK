...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 14:02:56 2024
...
from ACWE_python_fall_2023.ACWE_python_v3 import correct_limb_brightening as clb, acwe_exp3
...
saveFolder = saveFolder + '/HMI_Experiments/TestEvolutionMethods/Scaled/FluxImb/'
...
inunipolarityweight  = np.array([0,100])
outunipolarityweight = np.array([0,1/10])
epsilon = 0.01
alpha = [0.3,-1]
narrowband = 2
N = 10
...
# # Time ACWE
# timeFile = os.path.join(ROOT_DIR,'HMI_Experiments/TestEvolutionMethods/Scaled/')
# timeFile = timeFile + CR
# if resize_param == 8:
#     timeFile = timeFile + '_timeStandard_FluxImb.csv'
# else:
#     timeFile = timeFile + '_time' + acwePrefix + '_FluxImb.csv'
...
def itterateACWE2(I,im_size,sd_mask,m,foreground_weight=1,
                  ...
                  outunipolarityweight=1/10,epsilon = 0.01,
                  narrowband=2,N=10,fillInitHoles=True,verbose=False):
    ...
    while iterate:
        seg = acwe_exp3.acwe(I_seg,m_seg,N,(0,0,foreground_weight,
                                            background_weight,
                                            inunipolarityweight,
                                            outunipolarityweight,epsilon),
        ...
        # iteration, and now
        percent_diff = float(abs(seg_diff).sum())/\
                       float(seg.sum()+np.finfo(float).eps)*100.0
...
for i in range(len(data[keys[acweChoices[0]]])): # range(len(data[keys[acweChoices[0]]])-1, 0,-1): 
    ...
    if overwrite or not os.path.exists(crSaveFolder + acweFile):
        ...
        m          = np.zeros(Ishape).astype(int)        # Inital Masks
        ...
        for j in range(len(files)):
            ...
            # EUV
                ...
                if rollingAlpha != 0 and alpha[j] > 0:
                    sd_mask[:,:,j],m[:,:,j],alphar[j] = af6.inital_masks(Imini[:,:,j],
                                                                         Ishape[:-1],
                                                                         sun_radius[j],
                                                                         sun_center[j],
                                                                         alpha[j],
                    ...
                    sd_mask[:,:,j],m[:,:,j] = af6.inital_masks(Imini[:,:,j],
                                                               Ishape[:-1],
                                                               sun_radius[j],
                                                               sun_center[j],
                                                               alpha[j],
        ...
        seg = itterateACWE2(Imini,Ishape,sd_mask,m,foreground_weight,
                          ...
                          outunipolarityweight,epsilon,
                          narrowband,N,fillInitHoles,acweVerbose)
        ...
        as5.saveSeg(crSaveFolder + acweFile,seg,H,correctLimbBrightening,
                    ...
                    init_mask_method,fillInitHoles,alpha,alphar,narrowband,N)
