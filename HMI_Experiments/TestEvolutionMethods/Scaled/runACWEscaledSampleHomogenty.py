...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 24 09:56:59 2024
...
from ACWE_python_fall_2023.ACWE_python_v3 import correct_limb_brightening as clb, acwe_exp1
...
saveFolder = saveFolder + '/HMI_Experiments/TestEvolutionMethods/Scaled/Homogentiy/'
...
noScale = True
foreground_weight = np.array([1,1])
background_weight = np.array([1/50,1/50])
alpha = [0.3,-1]
narrowband = 2
N = 10
...
# if resize_param == 8:
#     timeFile = timeFile + '_timeStandard_Homogentiy.csv'
# else:
#     timeFile = timeFile + '_time' + acwePrefix + '_Homogentiy.csv'
...
def itterateACWE2(I,im_size,sd_mask,m,foreground_weight=1,
    ...
    while iterate:
        seg = acwe_exp1.acwe(I_seg,m_seg,N,(0,0,foreground_weight,
                                            background_weight),
...
for i in range(len(data[keys[acweChoices[0]]])): #range(len(data[keys[acweChoices[0]]])-1, 0,-1):
    ...
    if overwrite or not os.path.exists(crSaveFolder + acweFile):
        ...
        seg = itterateACWE2(Imini,Ishape,sd_mask,m,foreground_weight,
                          background_weight,narrowband,N,fillInitHoles,
                          acweVerbose)
