...
saveFolder = saveFolder + '/HMI_Experiments/TestSeedingMethods/Standard/Uni/'
...
def itterateACWE3(I,im_size,sd_mask,m,foreground_weight=1,
                  background_weight=1/50.,inunipolarityweight=100,
                  outunipolarityweight=1/10,narrowband=2,N=10,switch=50,
                  strell=5,uniCap=0.8,fillInitHoles=True,verbose=False):
    ...
    #  Exit if no CHs were found
    if np.sum(m_seg.astype(int)) == 0:
        return m_seg
...
for i in range(len(data[keys[acweChoices[0]]])): #range(len(data[keys[acweChoices[0]]])-1, 0,-1):
    ...
    if overwrite or not os.path.exists(crSaveFolder + acweFile):
        ...
        # RUN ACWE
        seg = itterateACWE3(Imini,Ishape,sd_mask,m,foreground_weight,
                            background_weight,inunipolarityweight,
                            outunipolarityweight,narrowband,N,switch,
                            strell,uniCap,fillInitHoles,acweVerbose)
