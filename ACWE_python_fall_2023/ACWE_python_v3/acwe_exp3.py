...
#     pp. 410-438, 1999.
# [5] I. F. Scholl & S. R. Habbal, "Automated Detection and Classification of
#     Coronal Holes and Filaments Based on EUV and Magnetogram Observations of
#     the Solar Disk," Solar Physics, vol. 293, no. 4, p. 71, 2008
# 
# Notes:
...
#          Oct 12, 2023: Image Force Test - Flux Imbalence, based on, [5]
...
def acwe(I,m,N,weights,narrowband,plot_progress):
    ...
    while (counter<N and iterate):
        ...
        if len(I.shape) == 2:
            ...
            # New Image Force - Flux Imbalence Foreground
            segSize = seg.astype(int).sum()
            mabs    = abs(I[seg]).mean() * segSize /  (segSize + 1)
            mnom    = I[seg].mean() * segSize /  (segSize + 1)
            fluximb = (mnom + I[abs(phi)<=narrowband]/(segSize + 1))/\
                      (mabs + abs(I[abs(phi)<=narrowband])/(segSize + 1))
            F_image = -lambda_u / (abs(fluximb) + epsilon) + F_image
            
            # Flux Imbalence Background
            segSize = ~seg.astype(int).sum()
            mabs    = abs(I[~seg]).mean() * segSize /  (segSize + 1)
            mnom    = I[~seg].mean() * segSize /  (segSize + 1)
            fluximb = (mnom + I[abs(phi)<=narrowband]/(segSize + 1))/\
                      (mabs + abs(I[abs(phi)<=narrowband])/(segSize + 1))
            F_image = lambda_m / (abs(fluximb) + epsilon) + F_image
        ...
        # Vector-Valued Image [2]
            ...
            for i in range(array(I.shape)[2]):
                ...
                fluximb = (mnom + Imini[abs(phi)<=narrowband]/(segSize + 1))/\
                          (mabs + abs(Imini[abs(phi)<=narrowband])/(segSize + 1))
                F_image = -lambda_u[i] / (abs(fluximb) + epsilon) + F_image
                ...
                fluximb = (mnom + Imini[abs(phi)<=narrowband]/(segSize + 1))/\
                          (mabs + abs(Imini[abs(phi)<=narrowband])/(segSize + 1))
                F_image = lambda_m[i] / (abs(fluximb) + epsilon) + F_image
