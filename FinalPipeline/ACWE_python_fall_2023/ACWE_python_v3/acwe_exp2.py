...
#     pp. 410-438, 1999.
# [5] Y.-K. Ko, K. Muglach, Y.-M. Wang, P. R. Young, & S. T. Lepri, "Temporal
...
#          Sep 28, 2023: Image Force Test - Uniplarity of [5]
...
def acwe(I,m,N,weights,narrowband,plot_progress):
    ...
    while (counter<N and iterate):
        ...
        if len(I.shape) == 2:
            ...
            # New Image Forces - Unipolarity Foreground
            segSize = seg.astype(int).sum()
            mabs    = abs(I[seg]).mean() * segSize /  (segSize + 1)
            mnom    = I[seg].mean() * segSize /  (segSize + 1)
            F_image = -lambda_u*(((mabs + abs(I[abs(phi)<=narrowband])/(segSize + 1)) - \
                                   abs(mnom + I[abs(phi)<=narrowband]/(segSize + 1))) / \
                                   (mabs + abs(I[abs(phi)<=narrowband])/(segSize + 1))) + \
                      F_image
                      
            # Unipolarity Background
            segSize = ~seg.astype(int).sum()
            mabs    = abs(I[~seg]).mean() * segSize /  (segSize + 1)
            mnom    = I[~seg].mean() * segSize /  (segSize + 1)
            F_image = lambda_m*(((mabs + abs(I[abs(phi)<=narrowband])/(segSize + 1)) - \
                                  abs(mnom + I[abs(phi)<=narrowband]/(segSize + 1))) / \
                                  (mabs + abs(I[abs(phi)<=narrowband])/(segSize + 1))) + \
                      F_image
        ...
        # Vector-Valued Image [2]
            ...
            for i in range(array(I.shape)[2]):
                ...
                mabs    = abs(Imini[~seg]).mean() * segSize /  (segSize + 1)
                mnom    = Imini[~seg].mean() * segSize /  (segSize + 1)
                F_image = lambda_m[i]*(((mabs + abs(Imini[abs(phi)<=narrowband])/(segSize + 1)) - \
                                         abs(mnom + Imini[abs(phi)<=narrowband]/(segSize + 1))) / \
                                         (mabs + abs(Imini[abs(phi)<=narrowband])/(segSize + 1))) + \
                          F_image
        F_length = mu*0
