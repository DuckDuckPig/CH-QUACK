...
#          Oct 12, 2023: Image Force Test - skew

from scipy.ndimage.morphology import distance_transform_edt
from numpy import log10, array, sqrt, zeros, max, hstack, tile, vstack
    ...
    while (counter<N and iterate):
        ...
        # Vector-Valued Image [2]
            ...
            for i in range(array(I.shape)[2]):
                ...
                # Append Placeholder
                segInt = hstack([segInt,0])
                segshape = len(segInt)
                
                # Tile and Introduce pixel we are evaluating
                segInt = tile(segInt,[len(array(Imini[abs(phi)<=narrowband])),1])
                segInt[:,-1] = array(Imini[abs(phi)<=narrowband])
                
                # Mean & STD of Ensemble
                segIntm   = segInt.mean(axis=1)
                segIntstd = segInt.std(axis=1)
                
                # Subtract mean
                segtmp = tile(vstack(segIntm),[1,segshape])
                segInt = segInt - segtmp
                
                # Divide by std and cube
                segtmp = tile(vstack(segIntstd),[1,segshape])
                segInt = (segInt/segtmp)**3
                
                # Sum and Weigh
                segInt = segInt.sum(axis=1)/segshape
                
                # Apply force
                F_image = F_image -lambda_u[i]/(abs(segInt) + epsilon)
                ...
                # Apply force
                F_image = F_image +lambda_m[i]/(abs(segInt) + epsilon)
