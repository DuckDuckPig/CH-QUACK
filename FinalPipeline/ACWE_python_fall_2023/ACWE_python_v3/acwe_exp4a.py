...
from numpy import log10, array, sqrt, zeros, max, hstack
    ...
    while (counter<N and iterate):
        ...
        # Vector-Valued Image [2]
            ...
            for i in range(array(I.shape)[2]):
                ...
                Isub   = Imini[abs(phi)<=narrowband]
                for j in range(len(F_image)):
                    temp = hstack([segInt,Isub[j]]).flatten()
                    temp = skew(temp)
                    F_image[j] = F_image[j] -lambda_u[i]/(abs(temp) + epsilon)
                ...
                for j in range(len(F_image)):
                    temp = hstack([segInt,Isub[j]]).flatten()
                    temp = skew(temp)
                    F_image[j] = F_image[j] +lambda_m[i]/(abs(temp) + epsilon)
