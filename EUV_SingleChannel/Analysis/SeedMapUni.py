...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 14:34:24 2024
...
uniDataFolder = GeneralFolder + 'StandardSeedStats/' # 'ScaledSeedStats/'
...
for i in range(len(data)):#-1,-1,-1):
    ...
    if overwrite or not os.path.exists(crUniFolder + uniFile):
        ...
        # Open Confidence Map
        H,AH,_ = acweSaveSeg_v5.openSeg(crSaveFolder+acweFile)
        
        # Upscale
        ResizeParam = AH['RESIZE_PARAM']
        if ResizeParam != 1:
            SEED = acweRestoreScale.upscale(AH['INIT_MASK'], AH)
        else:
            SEED = AH['INIT_MASK']
        ...
        uni = np.empty([len(SEED2),2])
        uni[:] = np.nan
        
        # Cycle Through Regions
        for j in range(len(SEED2)):
        ...
        # Save Results
        np.savez_compressed(crUniFolder+uniFile,SEED2,uni)
